...
export function generateWorld(seed, mapNumber=1n, previous='', recent=[], boss=false, generation=7) {
 ...
 if(generation>=7)for(const tree of borderTrees(seed,riverX))for(let y=tree.y+1-tree.size;y<=tree.y;y++)for(let x=tree.x+1-tree.size;x<=tree.x;x++)world.blocked.add(`${x},${y}`);
 ...
 // Reserve large landmarks before settlements, encounters and small dressing.
 for(const kind of ['tree-large','tree-conifer-large','tree-crooked-large','rock-large','rock-layered-large']){
   const candidates=[];for(let y=3;y<HEIGHT-2;y++)for(let x=3;x<WIDTH-2;x++)if(clear(x-1,y-1,2,2))candidates.push({x,y});
   if(!candidates.length)continue;const p=pick(candidates);world.trees.push({...p,size:2,kind});
   for(let y=p.y-1;y<=p.y;y++)for(let x=p.x-1;x<=p.x;x++)world.blocked.add(`${x},${y}`);
 }
 for(let i=0;i<150&&world.buildings.length<6;i++) {
   const x=2+Math.floor(r()*19),y=2+Math.floor(r()*12);
   const kind=pick(['house','inn','tower','ruins','inn-small','tower-small']),size=kind.endsWith('-small')?1:2;
   if(!clear(x-1,y-1,size+2,size+2))continue;
   const building={kind,x,y,w:size,h:size};
   world.buildings.push(building);
   for(let yy=y;yy<y+size;yy++)for(let xx=x;xx<x+size;xx++)world.blocked.add(`${xx},${yy}`);
   if(kind==='house'||kind==='inn'||kind==='inn-small') {
     building.door={x:x+size-1,y:y+size};
 ...
 const reachable=new Set(),queue=[world.entrance];
 for(let i=0;i<queue.length;i++){const p=queue[i],k=`${p.x},${p.y}`;if(reachable.has(k))continue;reachable.add(k);for(const [dx,dy]of [[1,0],[-1,0],[0,1],[0,-1]]){const x=p.x+dx,y=p.y+dy;if(x>0&&x<WIDTH-1&&y>0&&y<HEIGHT-1&&tiles[y][x].kind!=='water'&&!world.blocked.has(`${x},${y}`)&&!reachable.has(`${x},${y}`))queue.push({x,y});}}
 const available=tiles.flat().filter(t=>reachable.has(`${t.x},${t.y}`)&&t.x>1&&t.x<WIDTH-2&&t.y>1&&t.y<HEIGHT-2&&t.x!==riverX&&!world.blocked.has(`${t.x},${t.y}`)&&!(t.x===left&&t.y===top)&&!world.buildings.some(b=>b.door&&b.door.x===t.x&&b.door.y===t.y));
 const used=new Set(), protectedCells=new Set();
 if(generation>=7){
   const exit=world.stops.find(o=>o.type==='exit');
   const candidates=available.filter(p=>!p.path&&p.x>2&&p.y>2&&clear(p.x-1,p.y-1,2,2));
   if(!candidates.length)throw Error('No clear exit footprint');
   const p=pick(candidates);Object.assign(exit,{x:p.x,y:p.y,size:2});
   for(let y=p.y-1;y<=p.y;y++)for(let x=p.x-1;x<=p.x;x++){used.add(`${x},${y}`);protectedCells.add(`${x},${y}`);}
 }

 // Shuffle placement order so quest and combat objects share the same ratio.
 const placements=world.objects.filter(o=>generation<7||o.type!=='exit');for(let i=placements.length-1;i>0;i--){const j=Math.floor(r()*(i+1));[placements[i],placements[j]]=[placements[j],placements[i]];}
...
// Perimeter trees do not replace interior scenery.
export function borderTrees(seed,riverX){
 const r=rng(seed^0xb04de),trees=[],positions=[];
 for(let x=0;x<WIDTH;x++){positions.push({x,y:0},{x,y:HEIGHT-1});}
 for(let y=1;y<HEIGHT-1;y++)positions.push({x:0,y},{x:WIDTH-1,y});
 for(const p of positions){if(Math.abs(p.x-riverX)<1.5)continue;
  const large=trees.length%11===10;
  trees.push({...p,...(large?{x:Math.max(1,p.x),y:Math.max(1,p.y)}:{}),size:large?2:1,kind:large?['tree-large','tree-conifer-large','tree-crooked-large'][Math.floor(r()*3)]:'tree-small'});
 }
 return trees.filter(t=>t.size===2||!trees.some(big=>big.size===2&&t.x>=big.x-1&&t.x<=big.x&&t.y>=big.y-1&&t.y<=big.y));
}
